...
module vegNrgFlux_module
  ...
  contains
   ...
   real(dp),intent(out)            :: dCanopyEvaporation_dCanWat      ! derivative in canopy evaporation w.r.t. canopy total water content (s-1)
   ...
   real(dp),intent(out)            :: dGroundEvaporation_dCanWat      ! derivative in ground evaporation w.r.t. canopy total water content (s-1)
  ...
  ! output: transpiration derivatives
   ...
   ! output: cross derivative terms
   real(dp),intent(out)            :: dCanopyNetFlux_dCanWat          ! derivative in net canopy fluxes w.r.t. canopy total water content (J kg-1 s-1)
   real(dp),intent(out)            :: dGroundNetFlux_dCanWat          ! derivative in net ground fluxes w.r.t. canopy total water content (J kg-1 s-1)
   ...
   real(dp)                       :: turbFluxCanair_dStateCanWat      ! turbulent exchange from the canopy air space to the atmosphere, after canopy total water content is perturbed (W m-2)
   ...
   real(dp)                       :: turbFluxCanopy_dStateCanWat      ! total turbulent heat fluxes from the canopy to the canopy air space, after canopy total water content is perturbed (W m-2)
   ...
   real(dp)                       :: turbFluxGround_dStateCanWat      ! total turbulent heat fluxes from the ground to the canopy air space, after canopy total water content is perturbed (W m-2)
   ...
   real(dp)                       :: latHeatCanEvap_dStateCanWat      ! canopy evaporation after canopy total water content is perturbed (W m-2)
   ...
   real(dp)                       :: dTurbFluxCanair_dCanWat          ! derivative in net canopy air space fluxes w.r.t. canopy total water content (J kg-1 s-1)
   ...
   real(dp)                       :: dTurbFluxCanopy_dCanWat          ! derivative in net canopy turbulent fluxes w.r.t. canopy total water content (J kg-1 s-1)
   ...
   real(dp)                       :: dTurbFluxGround_dCanWat          ! derivative in net ground turbulent fluxes w.r.t. canopy total water content (J kg-1 s-1)
  
   ! (liquid water flux derivatives -- canopy evap)
   real(dp)                       :: dLatHeatCanopyEvap_dCanWat       ! derivative in latent heat of canopy evaporation w.r.t. canopy total water content (W kg-1)
   ...
   real(dp)                       :: dLatHeatGroundEvap_dCanWat       ! derivative in latent heat of ground evaporation w.r.t. canopy total water content (J kg-1 s-1)
   ...
   select case(ix_bcUpprTdyn)
    ...
    case(prescribedTemp,zeroFlux)
     ...
     dCanopyEvaporation_dCanWat = 0._dp    ! derivative in canopy evaporation w.r.t. canopy total water content (s-1)
     ...
     dGroundEvaporation_dCanWat = 0._dp    ! derivative in ground evaporation w.r.t. canopy total water content (s-1)
    ...
    case(energyFlux)
     ...
     if(computeVegFlux)then
      ...
      dCanopyWetFraction_dWat = 0._dp  ! derivative in wetted fraction w.r.t. canopy total water (kg-1 m2)
     ...
     do itry=nFlux,1,-1  ! (work backwards to ensure all computed fluxes come from the un-perturbed case)
      ...
      canopyIce = (1._dp - fracLiquidCanopy)*canopyWat
   ...
   ! output: liquid flux derivatives (canopy evap)
   real(dp),intent(out)          :: dLatHeatCanopyEvap_dCanWat   ! derivative in latent heat of canopy evaporation w.r.t. canopy total water content (W kg-1)
   ...
   ! output: liquid flux derivatives (ground evap)
   real(dp),intent(out)          :: dLatHeatGroundEvap_dCanWat   ! derivative in latent heat of ground evaporation w.r.t. canopy total water content (J kg-1 s-1)
   ...
   real(dp)                      :: dLatHeatCanopyTrans_dTCanair ! derivative in the latent heat of canopy transpiration w.r.t. canopy air temperature
   real(dp)                      :: dLatHeatCanopyTrans_dTCanopy ! derivative in the latent heat of canopy transpiration w.r.t. canopy temperature
   real(dp)                      :: dLatHeatCanopyTrans_dTGround ! derivative in the latent heat of canopy transpiration flux w.r.t. ground temperature
   ! output: cross derivatives
   real(dp),intent(out)          :: dTurbFluxCanair_dCanWat      ! derivative in net canopy air space fluxes w.r.t. canopy total water content (J kg-1 s-1)
   real(dp),intent(out)          :: dTurbFluxCanopy_dCanWat      ! derivative in net canopy turbulent fluxes w.r.t. canopy total water content (J kg-1 s-1)
   real(dp),intent(out)          :: dTurbFluxGround_dCanWat      ! derivative in net ground turbulent fluxes w.r.t. canopy total water content (J kg-1 s-1)
   ! output: error control
   integer(i4b),intent(out)      :: err                          ! error code
   character(*),intent(out)      :: message                      ! error message
   ! -----------------------------------------------------------------------------------------------------------------------------------------
   ...
   real(dp)                      :: dVPCanopyAir_dCanWat         ! derivative of vapor pressure in the canopy air space w.r.t. canopy total water content
   ...
   if(ixDerivMethod == analytical)then
    ...
    if(computeVegFlux)then
     ...
     dLatHeatCanopyEvap_dCanWat   = 0._dp
     dLatHeatCanopyTrans_dCanWat  = 0._dp
     dVPCanopyAir_dCanWat         = 0._dp
   ...
   if(ixDerivMethod == analytical)then
    ...
    ! (cross deriavtives)
    dTurbFluxCanair_dCanWat  = 0._dp                                                                                 ! derivative in net canopy air space fluxes w.r.t. canopy total water content (J kg-1 s-1)
    ...
    ! (liquid water derivatives)
    dLatHeatCanopyEvap_dCanWat   = 0._dp
    dLatHeatGroundEvap_dCanWat   = 0._dp
    ! (cross deriavtives)
    dTurbFluxCanair_dCanWat  = 0._dp
    dTurbFluxCanopy_dCanWat  = 0._dp
    dTurbFluxGround_dCanWat  = 0._dp
   end if
