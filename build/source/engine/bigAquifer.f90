...
subroutine bigAquifer(&
  ...
  ! output: derivatives in transpiration w.r.t. canopy state variables
  real(dp),intent(inout)           :: dAquiferTrans_dTCanair      ! derivatives in the aquifer transpiration flux w.r.t. canopy air temperature
  real(dp),intent(inout)           :: dAquiferTrans_dTCanopy      ! derivatives in the aquifer transpiration flux w.r.t. canopy temperature
  real(dp),intent(inout)           :: dAquiferTrans_dTGround      ! derivatives in the aquifer transpiration flux w.r.t. ground temperature
  real(dp),intent(inout)           :: dAquiferTrans_dCanWat       ! derivatives in the aquifer transpiration flux w.r.t. canopy total water
  ! output: error control
