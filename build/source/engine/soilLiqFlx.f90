...
module soilLiqFlx_module
  ...
  contains
   ...
   real(dp),intent(in)              :: dCanopyTrans_dCanWat          ! derivative in canopy transpiration w.r.t. canopy total water content (s-1)
   real(dp),intent(in)              :: dCanopyTrans_dTCanair         ! derivative in canopy transpiration w.r.t. canopy air temperature (kg m-2 s-1 K-1)
   real(dp),intent(in)              :: dCanopyTrans_dTCanopy         ! derivative in canopy transpiration w.r.t. canopy temperature (kg m-2 s-1 K-1)
   real(dp),intent(in)              :: dCanopyTrans_dTGround         ! derivative in canopy transpiration w.r.t. ground temperature (kg m-2 s-1 K-1)
   real(dp),intent(in)              :: above_soilLiqFluxDeriv        ! derivative in layer above soil (canopy or snow) liquid flux w.r.t. liquid water
   real(dp),intent(in)              :: above_soildLiq_dTk            ! derivative of layer above soil (canopy or snow) liquid flux w.r.t. temperature
   real(dp),intent(in)              :: above_soilFracLiq             ! fraction of liquid water layer above soil (canopy or snow) (-)
   ! input: model fluxes
   ...
   real(dp),intent(inout)           :: dq_dHydStateLayerSurfVec(0:)  ! derivative in surface infiltration w.r.t. hydrology state in above soil snow or canopy and every soil layer  (m s-1 or s-1)
   ! output: derivatives in fluxes w.r.t. energy state variables -- now just temperature -- in the layer above and layer below (m s-1 K-1)
   ...
   real(dp),intent(inout)           :: dq_dNrgStateLayerSurfVec(0:)  ! derivative in surface infiltration w.r.t. temperature in above soil snow or canopy and every soil layer  (m s-1 or s-1)
   ! output: derivatives in transpiration w.r.t. canopy state variables
   real(dp),intent(inout)           :: mLayerdTrans_dTCanair(:)      ! derivatives in the soil layer transpiration flux w.r.t. canopy air temperature
   real(dp),intent(inout)           :: mLayerdTrans_dTCanopy(:)      ! derivatives in the soil layer transpiration flux w.r.t. canopy temperature
   real(dp),intent(inout)           :: mLayerdTrans_dTGround(:)      ! derivatives in the soil layer transpiration flux w.r.t. ground temperature
   real(dp),intent(inout)           :: mLayerdTrans_dCanWat(:)       ! derivatives in the soil layer transpiration flux w.r.t. canopy total water
   ! output: error control
   ...
   if( .not. (scalarSolution .and. ixTop>1) )then
    ...
    if(ixBcUpperSoilHydrology==prescribedHead) then
     ...
     ! derivatives in transpiration w.r.t. canopy state variables
     mLayerdTrans_dCanWat(:) = 0._dp
     mLayerdTrans_dTCanair(:)= 0._dp
     mLayerdTrans_dTCanopy(:)= 0._dp
     mLayerdTrans_dTGround(:)= 0._dp
    endif
   ...
   do itry=nFlux,0,-1  ! (work backwards to ensure all computed fluxes come from the un-perturbed case)
    ...
    dq_dHydStateBelow(0) = 0._dp
    dq_dNrgStateBelow(0) = 0._dp
   ...
   real(dp),intent(out)          :: dq_dHydStateVec(0:)       ! derivative in surface infiltration w.r.t. hydrology state in above soil snow or canopy and every soil layer (m s-1 or s-1)
   real(dp),intent(out)          :: dq_dNrgStateVec(0:)       ! derivative in surface infiltration w.r.t. energy state in above soil snow or canopy and every soil layer  (m s-1 K-1)
   ! output: error control
   integer(i4b),intent(out)         :: err                       ! error code
   character(*),intent(out)         :: message                   ! error message
   ! -----------------------------------------------------------------------------------------------------------------------------
   ...
   real(dp)                      :: dpart1(1:nSoil),dpart2(1:nSoil)     ! derivatives for different parts of a function
   real(dp)                      :: dfracCap(1:nSoil),dfInfRaw(1:nSoil)  ! derivatives for different parts of a function
  ! (head boundary condition)
   ...
   ! (derivatives)
   real(dp)                      :: dVolFracLiq_dWat(1:nSoil)  ! derivative in vol fraction of liquid w.r.t. water state variable in root layers
   real(dp)                      :: dVolFracIce_dWat(1:nSoil)  ! derivative in vol fraction of ice w.r.t. water state variable in root layers
   real(dp)                      :: dVolFracLiq_dTk(1:nSoil)   ! derivative in vol fraction of liquid w.r.t. temperature in root layers
   real(dp)                      :: dVolFracIce_dTk(1:nSoil)   ! derivative in vol fraction of ice w.r.t. temperature in root layers
   real(dp)                      :: dRootZoneLiq_dWat(1:nSoil)       ! derivative in vol fraction of scalar root zone liquid w.r.t. water state variable in root layers
   real(dp)                      :: dRootZoneIce_dWat(1:nSoil)       ! derivative in vol fraction of scalar root zone ice w.r.t. water state variable in root layers
   real(dp)                      :: dRootZoneLiq_dTk(1:nSoil)        ! derivative in vol fraction of scalar root zone liquid w.r.t. temperature in root layers
   real(dp)                      :: dRootZoneIce_dTk(1:nSoil)        ! derivative in vol fraction of scalar root zone ice w.r.t. temperature in root layers
   real(dp)                      :: dDepthWettingFront_dWat(1:nSoil) ! derivative in scalar depth of wetting front w.r.t. water state variable in root layers
   real(dp)                      :: dDepthWettingFront_dTk(1:nSoil)  ! derivative in scalar depth of wetting front w.r.t. temperature in root layers
   real(dp)                      :: dXMaxInfilRate_dWat(1:nSoil)     ! derivative in scalar max infiltration rate w.r.t. water state variable in root layers
   real(dp)                      :: dXMaxInfilRate_dTk(1:nSoil)      ! derivative in scalar max infiltration rate w.r.t. temperature in root layers
   real(dp)                      :: dInfilArea_dWat(0:nSoil)         ! derivative in scalar infiltration rate w.r.t. water state variable in canopy or snow and root layers
   real(dp)                      :: dInfilArea_dTk(0:nSoil)          ! derivative in scalar infiltration rate w.r.t. temperature in canopy or snow and root layers
   real(dp)                      :: dFrozenArea_dWat(0:nSoil)        ! derivative in scalar frozen area w.r.t. water state variable in canopy or snow and root layers
   real(dp)                      :: dFrozenArea_dTk(0:nSoil)         ! derivative in scalar frozen area w.r.t. temperature in canopy or snow and root layers
   real(dp)                      :: dInfilRate_dWat(0:nSoil)         ! derivative in scalar infiltration rate w.r.t. water state variable in canopy or snow and root layers
   real(dp)                      :: dInfilRate_dTk(0:nSoil)          ! derivative in scalar infiltration rate w.r.t. temperature in canopy or snow and root layers
  
   ! initialize error control
   err=0; message="surfaceFlx/"
  
   ! initialize derivatives
   dq_dHydStateVec(:) = 0._dp
   dq_dNrgStateVec(:) = 0._dp
   ...
   select case(bc_upper)
    ...
    case(prescribedHead)
     ...
     if(deriv_desired)then
      ...
      select case(ixRichards)  ! (form of Richards' equation)
       case(moisture); dq_dHydStateVec(1) = -surfaceDiffuse/(mLayerDepth(1)/2._dp)
       case(mixdform); dq_dHydStateVec(1) = -surfaceHydCond/(mLayerDepth(1)/2._dp)
       case default; err=10; message=trim(message)//"unknown form of Richards' equation"; return
      end select
      ! compute the energy derivative at the surface
      dq_dNrgStateVec(1) = -(dHydCond_dTemp/2._dp)*(scalarMatricHead - upperBoundHead)/(mLayerDepth(1)*0.5_dp) + dHydCond_dTemp/2._dp
    ...
    case(liquidFlux)
     ...
     if(doInfiltration)then
      ...
      rootZoneLiq = 0._dp
      rootZoneIce = 0._dp
      dRootZoneLiq_dWat(:) = 0._dp
      dRootZoneIce_dWat(:) = 0._dp
      dRootZoneLiq_dTk(:)  = 0._dp
      dRootZoneIce_dTk(:)  = 0._dp
      ...
      dPart1(:) = surfaceSatHydCond*(zScale_TOPMODEL - 1._dp) * ( (1._dp - depthWettingFront/sum(mLayerDepth))**(zScale_TOPMODEL - 2._dp) ) * (-dDepthWettingFront_dWat(:))/sum(mLayerDepth)
      dPart2(:) = -wettingFrontSuction / (dDepthWettingFront_dWat(:)**2._dp)
      dXMaxInfilRate_dWat(:)  = fPart1*dpart2(:) + fPart2*dPart1(:)
      dPart1(:) = surfaceSatHydCond*(zScale_TOPMODEL - 1._dp) * ( (1._dp - depthWettingFront/sum(mLayerDepth))**(zScale_TOPMODEL - 2._dp) ) * (-dDepthWettingFront_dTk(:))/sum(mLayerDepth)
      dPart2(:) = -wettingFrontSuction / (dDepthWettingFront_dTk(:)**2._dp)
      ...
      if(qSurfScale < qSurfScaleMax)then
       ...
       if (0.5_dp*(fInfRaw + sqrt(fInfRaw**2._dp + scaleFactor))< 1._dp) then
        dfracCap(:) = ( dRootZoneLiq_dWat(:)/maxFracCap + dRootZoneIce_dWat(:)*fracCap )/availCapacity
        dfInfRaw(:) = qSurfScale*dfracCap(:) * exp(-qSurfScale*(1._dp - fracCap))
        dInfilArea_dWat(1:nSoil) = 0.5_dp*dfInfRaw(:) * (1._dp + fInfRaw/sqrt(fInfRaw**2._dp + scaleFactor))
        dfracCap(:) = ( dRootZoneLiq_dTk(:)/maxFracCap + dRootZoneIce_dTk(:)*fracCap )/availCapacity
        dfInfRaw(:) = qSurfScale*dfracCap(:) * exp(-qSurfScale*(1._dp - fracCap))
        dInfilArea_dTk(1:nSoil)  = 0.5_dp*dfInfRaw(:) * (1._dp + fInfRaw/sqrt(fInfRaw**2._dp + scaleFactor))
       else ! scalarInfilArea = 1._dp
        dInfilArea_dWat(:) = 0._dp
        dInfilArea_dTk(:)  = 0._dp
       endif
      else
      ...
      dInfilArea_dWat(0) = 0._dp
      dInfilArea_dTk(0)  = 0._dp
      ...
      if(rootZoneIce > tiny(rootZoneIce))then  ! (avoid divide by zero)
       ...
       dFrozenArea_dWat(:) = 0._dp
       dFrozenArea_dTk(:)  = 0._dp
      else
       scalarFrozenArea = 0._dp
       dFrozenArea_dWat(:) = 0._dp
       dFrozenArea_dTk(:)  = 0._dp
      end if
      dFrozenArea_dWat(0) = 0._dp
      dFrozenArea_dTk(0)  = 0._dp
     ...
     if (xMaxInfilRate < scalarRainPlusMelt) then ! = dXMaxInfilRate_d
      ...
      dInfilRate_dWat(:) = 0._dp !only calculate for layers that are not the surface
      dInfilRate_dTk(:)  = 0._dp !only calculate for layers that are not the surface
     ...
     dq_dHydStateVec(:) = (1._dp - scalarFrozenArea) * ( dInfilArea_dWat(:)*min(scalarRainPlusMelt,xMaxInfilRate) + scalarInfilArea*dInfilRate_dWat(:) ) +&
                          (-dFrozenArea_dWat(:))*scalarInfilArea*min(scalarRainPlusMelt,xMaxInfilRate)
     dq_dNrgStateVec(:) = (1._dp - scalarFrozenArea) * ( dInfilArea_dTk(:) *min(scalarRainPlusMelt,xMaxInfilRate) + scalarInfilArea*dInfilRate_dTk(:)  ) +&
