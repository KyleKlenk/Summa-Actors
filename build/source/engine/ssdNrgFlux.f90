...
module ssdNrgFlux_module
  ...
  contains
   ...
   real(dp),intent(inout)          :: dGroundNetFlux_dGroundTemp  ! derivative in net ground flux w.r.t. ground temperature (W m-2 K-1)
   ! input: liquid water fluxes
  ...
  ! input: pre-computed derivatives
   ...
   real(dp),intent(in)              :: mLayerFracLiqSnow(:)       ! fraction of liquid water (-)
   ...
   real(dp),intent(out)             :: dFlux_dWatAbove(0:)        ! derivatives in the flux w.r.t. water state in the layer above (J m-2 s-1 K-1)
   real(dp),intent(out)             :: dFlux_dWatBelow(0:)        ! derivatives in the flux w.r.t. water state in the layer below (J m-2 s-1 K-1)
   ! output: error control
   ...
   real(dp)                      :: scalarThermCFlux               ! thermal conductivity (W m-1 K-1)
   real(dp)                      :: scalarThermCFlux_dTempAbove    ! thermal conductivity with perturbation to the temperature state above (W m-1 K-1)
   real(dp)                      :: scalarThermCFlux_dTempBelow    ! thermal conductivity with perturbation to the temperature state below (W m-1 K-1)
   real(dp)                      :: scalarThermCFlux_dWatAbove     ! thermal conductivity with perturbation to the water state above
   real(dp)                      :: scalarThermCFlux_dWatBelow     ! thermal conductivity with perturbation to the water state below
   ...
   real(dp)                      :: matricFHead                  ! matric head for frozen soil
   ...
   real(dp),dimension(2)         :: vectorVolFracLiqTrial        ! trial value of volumetric liquid content (-)
   real(dp),dimension(2)         :: vectorVolFracIceTrial        ! trial value of volumetric ice content (-)
   real(dp),dimension(2)         :: vectorTempTrial              ! trial value of temperature (K)
   real(dp),dimension(2)         :: vectordTheta_dPsi            ! derivative in the soil water characteristic w.r.t. psi (m-1)
   real(dp),dimension(2)         :: vectordPsi_dTheta            ! derivative in the soil water characteristic w.r.t. theta (m)
   real(dp),dimension(2)         :: vectorFracLiqSnow            ! fraction of liquid water (-)
   real(dp),dimension(2)         :: vectortheta_sat              ! layer above and below soil porosity (-)
   real(dp),dimension(2)         :: vectoriden_soil              ! layer above and below density of soil (kg m-3)
   real(dp),dimension(2)         :: vectorthCond_soil            ! layer above and below thermal conductivity of soil (W m-1 K-1)
   real(dp),dimension(2)         :: vectorfrac_sand              ! layer above and below fraction of sand (-)
   real(dp),dimension(2)         :: vectorfrac_clay              ! layer above and below fraction of clay (-)
   ! recompute the perturbed version of iLayerThermalC, this could be the only version and remove the omputThermConduct_module
   real(dp)                      :: scalariLayerThermalC         ! thermal conductivity at the interface of each layer (W m-1 K-1)
   real(dp)                      :: dThermalC_dHydStateAbove     ! derivative in the thermal conductivity w.r.t. water state in the layer above
   real(dp)                      :: dThermalC_dHydStateBelow     ! derivative in the thermal conductivity w.r.t. water state in the layer above
   real(dp)                      :: dThermalC_dNrgStateAbove     ! derivative in the thermal conductivity w.r.t. energy state in the layer above
   real(dp)                      :: dThermalC_dNrgStateBelow     ! derivative in the thermal conductivity w.r.t. energy state in the layer above
   ! ------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   ! ***** the upper boundary
   dFlux_dTempAbove(0) = 0._dp ! this will be in canopy
   dFlux_dWatAbove(0) = 0._dp ! this will be in canopy
   ...
   do iLayer=ixTop,ixBot
    ...
    if(iLayer==0)then  ! (upper boundary)
     ...
     select case(ix_bcUpprTdyn)
      ...
      case(zeroFlux)
       dFlux_dWatBelow(iLayer) = 0._dp
       dFlux_dTempBelow(iLayer) = 0._dp
  
       ! * compute flux inside vegetation energy flux routine, use here
      case(energyFlux)
       dFlux_dWatBelow(iLayer) = 0._dp !dGroundNetFlux_dGroundWat, does not exist in vegNrgFlux
   ...
   real(dp),intent(in)           :: nodeMatricHead(:)         ! trial vector of total water matric potential (m)
   real(dp),intent(in)           :: nodeVolFracLiq(:)         ! trial vector of volumetric liquid water content, recomputed with perturbed water state(-)
   real(dp),intent(in)           :: nodeVolFracIce(:)         ! trial vector of ice content, recomputed with perturbed water state(-)
   real(dp),intent(in)           :: nodeTemp(:)               ! trial vector of temperature (K)
   ! input: pre-computed derivatives
   real(dp),intent(in)           :: dTheta_dTk(:)             ! derivative in volumetric liquid water content w.r.t. temperature (K-1)
   real(dp),intent(in)           :: fracLiqSnow(:)            ! fraction of liquid water (-)
   real(dp),intent(in)           :: dTheta_dPsi(:)            ! derivative in the soil water characteristic w.r.t. psi (m-1)
   real(dp),intent(in)           :: dPsi_dTheta(:)            ! derivative in the soil water characteristic w.r.t. theta (m)
   ! input: model coordinate variables
   real(dp),intent(in)           :: nodeHeight(:)             ! height at the mid-point of the lower node (m)
   real(dp),intent(in)           :: node_iHeight(:)           ! height at the interface of each node (m)
   ! input: soil parameters
   ...
   real(dp),intent(in)           :: frac_sand(:)              ! intent(in): fraction of sand (-)
   real(dp),intent(in)           :: frac_clay(:)              ! fraction of clay (-)
   ! input: snow parameters
   real(dp),intent(in)           :: fixedThermalCond_snow     ! intent(in): temporally constant thermal conductivity of snow (W m-1 K-1)
   ! output: thermal conductivity at layer interfaces
   real(dp),intent(out)          :: iLayerThermalC            ! thermal conductivity at the interface of each layer (W m-1 K-1)
   ! output: thermal conductivity derivatives at all layer interfaces
   real(dp),intent(out)          :: dThermalC_dHydStateAbove  ! derivatives in the thermal conductivity w.r.t. matric head or volumetric liquid water in the layer above
   real(dp),intent(out)          :: dThermalC_dHydStateBelow  ! derivatives in the thermal conductivity w.r.t. matric head or volumetric liquid water in the layer below
   real(dp),intent(out)          :: dThermalC_dNrgStateAbove  ! derivatives in the thermal conductivity w.r.t. temperature in the layer above (W m-1 K-2)
   real(dp),intent(out)          :: dThermalC_dNrgStateBelow  ! derivatives in the thermal conductivity w.r.t. temperature in the layer below (W m-1 K-2)
   ! output: error control
   integer(i4b),intent(out)         :: err                       ! error code
   character(*),intent(out)         :: message                   ! error message
   ! --------------------------------------------------------------------------------------------------------------------------------
   ...
   real(dp)                      :: dThermalC_dWat(2)   ! derivative in thermal conductivity w.r.t. matric head or volumetric liquid water
   real(dp)                      :: dThermalC_dNrg(2)   ! derivative in thermal conductivity w.r.t. temperature
   real(dp)                      :: Tcrit                     ! temperature where all water is unfrozen (K)
   real(dp)                      :: dlambda_wet_dWat          ! derivative in thermal conductivity of wet material w.r.t.soil water state variable
   real(dp)                      :: dlambda_wet_dTk           ! derivative in thermal conductivity of wet material w.r.t. temperature
   real(dp)                      :: dkerstenNum_dWat          ! derivative in Kersten number w.r.t. soil water state variable
   real(dp)                      :: mLayerThermalC(2)         ! thermal conductivity of each layer (W m-1 K-1)
   real(dp)                      :: dVolFracLiq_dWat    ! derivative in vol fraction of liquid w.r.t. water state variable
   real(dp)                      :: dVolFracIce_dWat    ! derivative in vol fraction of ice w.r.t. water state variable
   real(dp)                      :: dVolFracLiq_dTk     ! derivative in vol fraction of liquid w.r.t. temperature
   real(dp)                      :: dVolFracIce_dTk     ! derivative in vol fraction of ice w.r.t. temperature
  ! local variables to reproduce the thermal conductivity of Hansson et al. VZJ 2005
   ...
   real(dp)                      :: dxArg_dWat,dxArg_dTk      ! derivates of the temporary variables with respect to soil water state variable and temperature
   ...
   do iLayer=ixUpper,ixLower
    ...
    if(ixThCondSoil==funcSoilWet .and. layerType(iLayer)==iname_soil)then
     bulkden_soil   = iden_soil(iLayer)*( 1._dp - theta_sat(iLayer) )
     lambda_drysoil = (0.135_dp*bulkden_soil + 64.7_dp) / (iden_soil(iLayer) - 0.947_dp*bulkden_soil)
     lambda_wetsoil = (8.80_dp*frac_sand(iLayer) + 2.92_dp*frac_clay(iLayer)) / (frac_sand(iLayer) + frac_clay(iLayer))
    end if
  
    ! *****
    ! * compute the thermal conductivity of snow and soil and derivates at the mid-point of each layer...
    ! ***************************************************************************************************
    dThermalC_dWat(iLayer) = 0._dp
    dThermalC_dNrg(iLayer) = 0._dp
    ...
    select case(layerType(iLayer))
     ...
     case(iname_soil)
      ...
      ! (process derivatives)
      dVolFracLiq_dWat = 0._dp
      dVolFracIce_dWat = 0._dp
      dVolFracLiq_dTk  = 0._dp
      dVolFracIce_dTk  = 0._dp
      if(deriv_desired)then
       select case(ixRichards)  ! (form of Richards' equation)
        case(moisture)
         dVolFracLiq_dWat = 1._dp
         dVolFracIce_dWat = dPsi_dTheta(iLayer) - 1._dp
        case(mixdform)
      ...
      select case(ixThCondSoil)
       ...
       case(funcSoilWet)
        ...
        lambda_wet  = lambda_wetsoil**( 1._dp - theta_sat(iLayer) ) * lambda_water**theta_sat(iLayer) * lambda_ice**(theta_sat(iLayer) - nodeVolFracLiq(iLayer))
        ...
        if(relativeSat > 0.1_dp)then ! log10(0.1) = -1
         ...
         dkerstenNum_dWat = (dVolFracIce_dWat + dVolFracLiq_dWat) / ( theta_sat(iLayer) * relativeSat * log(10._dp) )
        else
         kerstenNum = 0._dp  ! dry thermal conductivity
         dkerstenNum_dWat = 0._dp
        endif
       ...
       case(mixConstit)
        mLayerThermalC(iLayer) = thCond_soil(iLayer) * ( 1._dp - theta_sat(iLayer) ) + & ! soil component
       ...
       case(hanssonVZJ)
        fArg  = 1._dp + f1*nodeVolFracIce(iLayer)**f2
        xArg  = nodeVolFracLiq(iLayer) + fArg*nodeVolFracIce(iLayer)
        dxArg_dWat = dVolFracLiq_dWat + dVolFracIce_dWat * (1._dp + f1*(f2+1)*nodeVolFracIce(iLayer)**f2)
     ...
     ! ***** snow
     case(iname_snow)
      dVolFracIce_dWat = ( 1._dp - fracLiqSnow(iLayer) )*(iden_water/iden_ice)
      ...
      ! thermal conductivity as a function of snow density
       ...
       select case(ixThCondSnow)
        case(Yen1965)
         dThermalC_dWat(iLayer) = 2._dp * 3.217d-6 * nodeVolFracIce(iLayer) * iden_ice**2._dp * dVolFracIce_dWat
         dThermalC_dNrg(iLayer) = 2._dp * 3.217d-6 * nodeVolFracIce(iLayer) * iden_ice**2._dp * dVolFracIce_dTk
        case(Mellor1977)
         dThermalC_dWat(iLayer) = 2._dp * 2.576d-6 * nodeVolFracIce(iLayer) * iden_ice**2._dp * dVolFracIce_dWat
         dThermalC_dNrg(iLayer) = 2._dp * 2.576d-6 * nodeVolFracIce(iLayer) * iden_ice**2._dp * dVolFracIce_dTk
        case(Jordan1991)
         dThermalC_dWat(iLayer) = ( 7.75d-5 + 2._dp * 1.105d-6 * nodeVolFracIce(iLayer) * iden_ice ) * (lambda_ice-lambda_air) * iden_ice * dVolFracIce_dWat
         dThermalC_dNrg(iLayer) = ( 7.75d-5 + 2._dp * 1.105d-6 * nodeVolFracIce(iLayer) * iden_ice ) * (lambda_ice-lambda_air) * iden_ice * dVolFracIce_dTk
   ...
   if (ixLayerDesired==0) then
    ...
    if(ixThCondSoil==hanssonVZJ)then
     iLayerThermalC = 28._dp*(0.5_dp*(node_iHeight(ixLower) - node_iHeight(ixUpper))) ! these are indices 1,0 since was passed with 0:1
     dThermalC_dHydStateBelow = 0._dp
     dThermalC_dNrgStateBelow = 0._dp
    else
